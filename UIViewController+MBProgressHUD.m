...
- (void)hideHUDWithCompletionMessage:(NSString *)message withComplete:(BOOL)success;
{
    NSString *name = success ? @"37x-Checkmark.png" : @"excla";
    self.progressHUD.dimBackground = NO;
    self.progressHUD.customView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:name]];
...
- (void)hideHUDWithCompletionMessage:(NSString *)message finishedHandler:(HUDFinishedHandler)finishedHandler
{
    ...
    [self hideHUDWithCompletionMessage:message withComplete:YES];
}
...
- (void)showTextWithMessage:(NSString *)message withComplete:(BOOL)success
{
     [self _showHUDWithMessage:message];
